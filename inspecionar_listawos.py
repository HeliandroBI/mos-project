...
if r3.status_code == 200:
    print(f"Total: {r3.json().get('value')}")
else:
    print(f"Erro: {r3.status_code} - {r3.text[:300]}")

print("\n=== ITENS MAIS RECENTES (Modified desc) ===")
r2 = requests.get(f"{SP_SITE}/_api/web/lists/getbytitle('{LIST_NAME}')/items?$top=3&$orderby=Modified desc", headers=headers)
...
print("\n=== ITENS MAIS ANTIGOS (Created asc) ===")
r4 = requests.get(f"{SP_SITE}/_api/web/lists/getbytitle('{LIST_NAME}')/items?$top=2&$orderby=Created asc", headers=headers)
