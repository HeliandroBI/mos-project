...
print("=== COLUNAS ===")
r = requests.get(
    f"{SP_SITE}/_api/web/lists/getbytitle('{LIST_NAME}')/fields"
...
if r.status_code != 200:
    ...
    for f in r.json().get("value", []):
        print(f"  {f['InternalName']:40} | {f['Title']:30} | {f['TypeAsString']}")

print("\n=== TOTAL DE ITENS ===")
r2 = requests.get(f"{SP_SITE}/_api/web/lists/getbytitle('{LIST_NAME}')/itemcount", headers=headers)
if r2.status_code == 200:
    print(f"Total: {r2.json().get('value')}")

print("\n=== EXEMPLO (3 primeiros itens) ===")
r3 = requests.get(
    f"{SP_SITE}/_api/web/lists/getbytitle('{LIST_NAME}')/items?$top=3&$orderby=ID asc",
    headers=headers
)
if r3.status_code == 200:
    for it in r3.json().get("value", []):
